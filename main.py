...
def testArgs(args):
    ...
    if not os.path.isfile(inputHanziFile):
        printMsg("输入的词语文件不存在，请用全路径，或者相对于当前程序的相对路径。")
        sys.exit()
...
def genTest(phrasesStream):
    ...
    if len(line) > 0:
        lines.append(line)
    pages.append({"title": title, "lines": lines, "cur": curPage+1}) #last page
...
def genPractise(phrasesStream):
    ...
    pages = [] #object {'title':'', 'lines':[[{'py':'','hz':''},{'py':'','hz':''}]], 'cur':1} 的list
    title = '' #当前页上的标题
    lines = [] #临时保持当前页的行数据 
...
def main():
    ...
    parser.add_argument('-e', "--example", action="store_true", help="输入词列表的例子，供参考")
    parser.add_argument('-f', "--infile", type=str, default="./hanz.txt", help="汉字或者词语的列表文件，如果需要查看文件格式，制定 -e 或者 --example 参数查看输出的内容")
...
千里挑一 开开心心'''
    ...
    printMsg("生成完毕，请(用IE浏览器或者微信的Edge浏览器)打开当前目录下的{}.html 进行打印或者另存为pdf后打印。".format(args.style))
